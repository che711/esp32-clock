...
// Рабочий уровень разошёлся с выбранным — и почему. Нужно дашборду: без
// причины зафиксированный эконом с подсвеченным «normal» выглядит как не
// сработавшая кнопка, а не как «вернём, когда обстоятельство отпадёт».
bool        powerIsHeld();        // mode != chosen
const char* powerHoldReason();    // "" | "stopwatch" | "low" | "charged"
