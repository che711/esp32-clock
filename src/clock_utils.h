...
// уводится и ниже — до 1/4096, — но там она уже неразличима. Берём ночной
// уровень: это и есть самое тусклое, что проект считает читаемым.
//
// Раньше на это число опирался displaySetPower(): включение при нулевой
// яркости молча подтягивало уровень сюда, чтобы команда дала видимый
// результат. Подтягивание убрано — оно отменяло выбор пользователя и врало
...
// ─── Проценты ↔ уровень шкалы ─────────────────────────────
// Проценты здесь — доля ПЕРЦЕПТИВНОЙ шкалы, то есть положение ползунка,
// а не доля тока панели: ток из уровня считает panelDriveForLevel() по гамме
// (display_calc.h). «50 %» означает «половина яркости на глаз», и это ровно то
// число, которое пользователь выставил, — потому оно и показывается в дашборде.
//
// Оба пересчёта округляют, а не отбрасывают дробную часть. С отбрасыванием
// путь туда-обратно терял единицу на каждом обороте: 50 → 127 → 49 → 124 → 48.
// Дашборд показывал 49 % сразу после установки 50, а скрипт, читающий
// brightness_pct и пишущий его обратно, медленно уводил яркость вниз.

// уровень(0..255) → проценты(0..100)
inline uint8_t brightnessPct(uint8_t contrast) {
    return (uint8_t)(((uint32_t)contrast * 100 + CONTRAST_MAX / 2) / CONTRAST_MAX);
}

// проценты(0..100) → уровень(0..255)
inline uint8_t pctToContrast(int pct) {
    if (pct < 0)   pct = 0;
    if (pct > 100) pct = 100;
    // Ноль обязан остаться нулём: это «выключить», а не «почти не светит».
    // Округление его не сдвигает — 50/100 в целочисленном делении даёт 0.
    return (uint8_t)(((uint32_t)pct * CONTRAST_MAX + 50) / 100);
}
