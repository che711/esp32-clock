...
//
//  ┌─ КАРТА ПИНОВ ESP32-C6-Zero (только боковые гребёнки) ─┐
//  │ ЛЕВАЯ гребёнка:  5V GND 3V3 | 0 1 2 3 4 5             │
//  │   GPIO2 → Battery ADC  (делитель 18650)               │
//  │   GPIO4 → BMP280 SDA   (strapping, для I²C безопасно)  │
//  │   GPIO5 → BMP280 SCL   (strapping, для I²C безопасно)  │
//  │                                                        │
//  │ ПРАВАЯ гребёнка: TX RX 14 15 | 18 19 20 21 22 23      │
//  │   GPIO18 → OLED CLK  (пин 4 модуля)                    │
//  │   GPIO19 → OLED DIN  (пин 5)                           │
//  │   GPIO20 → OLED DC   (пин 14)                          │
//  │   GPIO21 → OLED RST  (пин 15)                          │
//  │   GPIO22 → OLED CS   (пин 16)                          │
//  │                                                        │
//  │ GPIO8 → WS2812 RGB LED (распаян на плате, не трогать)  │
//  └───────────────────────────────────────────────────────┘
//  НЕ на гребёнках (нижний торцевой ряд): 6,7,8,9,12,13,23.
//  Strapping-пины C6: 4,5,8,9,15 — под дисплей не брать.
//  GPIO12/13 = USB, GPIO16/17 = UART0 (TX/RX) — не занимать.
// ============================================================
...
// Все пять сигналов — на ПРАВОЙ гребёнке, подряд (удобно паять шлейфом).
// GPIO6/7 сюда не годятся: они в нижнем торцевом ряду, не на гребёнке.
...
// Кадр секундомера: полный занимает ~24 мс, но между секундами обновляется
// только полоска ".mmm" (~2 мс), поэтому 40 мс = 25 fps процессору по силам.
#define SW_DRAW_INTERVAL_MS     40UL  // перерисовка OLED при работе секундомера
