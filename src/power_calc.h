...
// Рубеж в том виде, в каком он применяется, — с гистерезисом. Мгновенное
// сравнение выше дребезжало: погасшая панель снимала с банки свою нагрузку,
// напряжение отскакивало на десятки милливольт (внизу кривой это несколько
// пунктов), рубеж снова пропускал, панель зажигалась, проседала и гасла опять.
// Поэтому погашенная панель ждёт не «выше offPct», а onPct — запаса, который
// отскоком не набирается (обоснование числа — у POWER_SCREEN_ON_PCT).
//
// wasOk — ответ прошлого вызова. При тех же входах ответ не меняется, так что
// спрашивать можно сколько угодно раз подряд — при условии onPct > offPct.
// Без батареи рубежа нет в обе стороны, как и в powerScreenBatteryOk().
inline bool powerScreenBatteryNext(bool wasOk, uint8_t pct, bool valid,
                                   uint8_t offPct, uint8_t onPct) {
    if (wasOk) return powerScreenBatteryOk(pct, valid, offPct);
    return !valid || pct >= onPct;
}
