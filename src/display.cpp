...
// Питание панели и только оно. Яркость эта функция не трогает: при нулевом
// уровне панель останется тёмной, и это правильный ответ — ноль ползунка
// пользователь поставил сам, и он значит «выключено».
//
// Раньше здесь стоял молчаливый подъём до CONTRAST_MIN_VISIBLE, чтобы команда
// «включить» давала видимый результат. Видимый она давала, но ценой вранья:
// уровень уезжал с 0 % на 28 %, дашборд об этом не узнавал (ползунок в ручном
// режиме со снимком не сверяется) и продолжал показывать 0 при светящейся
...
static void drawBottomStatus(const char* mark, bool withDate, bool withTemp) {
    ...
    // Адрес — последним: смотрят на него редко (открыть дашборд, когда роутер
    // выдал новый), поэтому при нехватке места жертвуем именно им. Другого
    // способа узнать IP с самих часов нет — mDNS убран.
