...
// Проценты считаем не по абсолютному заряду банки, а по тому, что реально
// доступно этому устройству. Между банкой и чипом стоят диод и LDO, и ниже
// ~3.6 В на банке 3.3 В на выходе уже не удержать — часы отключатся,
// сколько бы «настоящих» процентов в элементе ни оставалось. Поэтому хвост
// кривой поджат: 3.70 В — это 5 %, то есть «вот-вот всё».
inline uint8_t batteryVoltageToPercent(float v) {
    ...
    static const struct { float v; uint8_t pct; } CURVE[] = {
        ...
        {3.74f, 20},  {3.72f, 10}, {3.70f, 5},  {3.55f, 0},
    };
